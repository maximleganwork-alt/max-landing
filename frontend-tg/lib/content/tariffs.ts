import type { TariffsContent } from "shared/lib/content/tariffs";

export const tariffsContent: TariffsContent = {
  title: "Тарифы",
  lead: "Цена зависит от сложности логики, количества интеграций и объёма AI-функционала. Все цены — стартовые, итоговая стоимость фиксируется в договоре после составления ТЗ.",
  plans: [
    {
      id: "starter",
      title: "Старт",
      subtitle: "Простой бот с базовой логикой",
      price: "от 15 000 ₽",
      duration: "3–5 дней",
      features: [
        "Сценарий до 15 шагов диалога",
        "Меню, инлайн-кнопки, FAQ-ответы",
        "Сбор заявок и контактов",
        "Уведомления администратору",
        "Публикация через @BotFather",
        "Гарантия 30 дней",
      ],
      cta: "Выбрать Старт",
      goal: "tariff_select_starter",
    },
    {
      id: "business",
      title: "Бизнес",
      subtitle: "Бот или Mini App с AI и оплатами",
      price: "от 40 000 ₽",
      duration: "7–15 дней",
      features: [
        "Всё из тарифа «Старт»",
        "Сложный сценарий, ветвления, состояния",
        "Интеграция с AI (GPT/Claude/GigaChat)",
        "Оплаты: Telegram Stars, ЮKassa, СБП",
        "Подключение 1 внешней системы (CRM/API)",
        "Mini App или расширенный бот",
        "Базовая аналитика и метрики",
        "Гарантия 30 дней",
      ],
      cta: "Выбрать Бизнес",
      popular: true,
      goal: "tariff_select_business",
    },
    {
      id: "enterprise",
      title: "Корпоративный",
      subtitle: "Сложные системы для крупных проектов",
      price: "от 100 000 ₽",
      duration: "от 20 дней",
      features: [
        "Всё из тарифа «Бизнес»",
        "Сложная бизнес-логика и мульти-сценарии",
        "Глубокие AI-интеграции: RAG, fine-tuning, голос, vision",
        "Множественные интеграции (CRM, ERP, БД, очереди)",
        "Высокие нагрузки, отказоустойчивость",
        "Расширенная аналитика и дашборды",
        "Приоритетная поддержка и SLA",
        "Гарантия 30 дней",
      ],
      cta: "Обсудить проект",
      goal: "tariff_select_enterprise",
    },
  ],
  footnote:
    "Нужна поддержка после запуска? Подключаем по отдельной договорённости — фиксируем объём работ и стоимость заранее.",
};
