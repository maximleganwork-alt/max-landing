...
- App Router, all routes are SSG except `/api/lead` (`force-dynamic`). `app/(marketing)/` holds `page.tsx`, `MarketingClient.tsx`, `privacy/page.tsx`, `consent/page.tsx`.
...
- `app/opengraph-image.tsx` and `app/twitter-image.tsx` call `renderSiteOgImage(siteConfig)` from `shared/lib/og-image.tsx`. Runtime is `edge`; the route fetches Inter from Google Fonts on cold start (will 500 if outbound HTTPS to `*.googleapis.com` / `*.gstatic.com` is blocked).
...
- `shared/components/MarketingHome.tsx` — `"use client"` page assembly. Takes `{ contents: SiteContents, heroVisual?: ReactNode }` and renders `Header → Hero → Services → Process → WhyUs → Tariffs → WhyPlatform → FAQ → LeadForm → Footer`. **This file is the section order — change it here, not per-frontend.** The section background pattern alternates `bg → bg-subtle → bg → bg-subtle…`; when reordering, also flip the section component's `className` (`section-padding` vs `section-padding bg-bg-subtle`) to keep the rhythm.
- `shared/components/sections/` — all section components (`Hero`, `Services`, `Process`, `WhyUs`, `Tariffs`, `WhyPlatform`, `FAQ`, `LeadForm`, `Footer`, `SmartCaptcha`). **All are dumb / props-driven.** No section reads from a content file directly; data is wired from `MarketingHome`.
...
- `internal/handlers/lead.go` builds a Markdown message; `escapeMarkdown` escapes `_*\`[`. The header uses a coloured marker (`🟣 MAX`, `🔵 TG`, `🟢 WEB`) chosen by `lead.Source` so all three sources can share a single Telegram chat without ambiguity.
...
The repo ships with placeholders the customer will replace before launch. **Don't treat any of these as real values:** `BotMax`, `*.example.ru`, `hello@example.ru`, `@botmax_studio`, `@botmax_studio_tg`, `ИП Иванов Иван Иванович`, the ИНН/ОГРНИП/address in `shared/lib/legal-entity.ts`. Pre-launch checklist is in `NEED.MD` (replace legal entity + domains; submit РКН ПДн notification + cross-border transfer notification for Telegram per 152-ФЗ).
