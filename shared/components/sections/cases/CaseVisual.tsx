...
export function CaseVisual({ kind, image, sizes, priority = false, label, className }: CaseVisualProps) {
	if (image) {
		return (
			<div
				className={cn(
					'relative aspect-[16/9] w-full overflow-hidden bg-bg-subtle',
					className
				)}
			>
				<Image
	...
	return (
		<div
			aria-hidden="true"
			className={cn('relative w-full bg-bg-subtle', className)}
		>
...
const visualMap: Record<
	CaseVisualKind,
	(props: { label?: string }) => ReactElement
> = {
	chat: ChatVisual,
	...
	return (
		<defs>
			...
			>
				<stop
					offset="0%"
					stopColor="var(--brand-grad-from)"
				/>
				<stop
					offset="100%"
					stopColor="var(--brand-grad-to)"
				/>
			</linearGradient>
			<filter
				id={`${id}-shadow`}
		...
		>
			...
			<g filter={`url(#${id}-shadow)`}>
				...
				{[0, 1, 2, 3].map(i => (
					<g
						key={i}
						transform={`translate(${116 + (i % 2) * 66} ${64 + Math.floor(i / 2) * 50})`}
					>
						<rect
							width="56"
							height="42"
			...
			{[0, 1, 2, 3].map(row =>
				[0, 1, 2, 3, 4, 5, 6].map(col => {
					...
					return (
						<g
							key={key}
							transform={`translate(${36 + col * 42} ${74 + row * 28})`}
						>
							<rect
								width="36"
								height="22"
								rx="6"
								fill={filled ? 'var(--accent-soft)' : 'var(--bg-subtle)'}
								stroke="var(--border)"
							/>
							{filled ? (
								<>
		...
		>
			...
			>
				...
				{[0, 1, 2].map(i => (
					<g
						key={i}
						transform={`translate(14 ${50 + i * 22})`}
					>
						<rect
							width="120"
							height="4"
